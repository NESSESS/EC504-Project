...
#include <chrono>
/*
set title "X-fast VS BST in Successor"
plot "data" u 1:4 w lp t "X-fast", "test.txt" u 1:5 w lp t "BST"
EOF
 */
using namespace std;
...
int main(int argc, char *argv[]) {
    double time_buffer[9]={0};
    uint64_t * data=NULL;
    int loop=1;
    int i=0;int j=0; int k=0;int t=0;
    int round=atoi(argv[1]);
    int gap=atoi(argv[2]); //couvert the argv to gap and round.

    chrono::time_point<chrono::steady_clock> start, stop; 
    chrono::duration<double> difference_in_time;
    double difference_in_seconds;  // Holds the final run time
    cout<<"# N\t\t"<<"BST_Insert\t\t"<<"Xfast_Insert\t\t"<<"Yfast_Insert\t\t"
    <<"BST_Search\t\t"<<"Xfast_Search\t\t"<<"Yfast_Search\t\t"
    <<"BST_Successor\t\t"<<"Xfast_Successor\t\t"<<"Yfast_Successor\t\t\n";
//  take the bst at first:
    int N =gap;
	...
	time_buffer[l]+=difference_in_seconds/N;
	l++;
	...
	l++;
      ...
      difference_in_time = stop - start;
      difference_in_seconds = double(difference_in_time.count());
      time_buffer[l]+=difference_in_seconds;
      l++;

      start = chrono::steady_clock::now();
      xtrie.lookup(data[N/2]);
      stop = chrono::steady_clock::now();
      difference_in_time = stop - start;
      difference_in_seconds = double(difference_in_time.count());
      time_buffer[l]+=difference_in_seconds;
